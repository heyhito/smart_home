...
void Widget::on_pushButton_pressed()
{
    ...
    audio = new Audio;
    //开始录音，录音后产生的文件存放的路径
    audio->startAudio("D:\\Embedded_learning\\smart_home_Project\\IOT\\LOT_Cilent_1\\1.pcm");
}
...
void Widget::on_pushButton_released()
{
    ...
    //开始识别录音内容
    speech my_speech;
    QString text = my_speech.speechIdentify("D:\\Embedded_learning\\smart_home_Project\\IOT\\LOT_Cilent_1\\1.pcm");
    ui->textEdit->setText(text);

    ui->pushButton->setText("按住说话");

    if(text.left(2)==QString("开灯"))
    {
        QJsonObject json;
    ...
    else if(text.left(2)==QString("关灯"))
    {
        QJsonObject json;
        if(kitchen_flag == 1)
        {
        ...
        {
            ui->textEdit->setText("灯已关");
        }
    }
    else if(text.left(2)==QString("报警"))
    {
        QJsonObject json;
        ...
        {
            ui->textEdit->setText("报警器已响");
        }
    }
    else if(text.left(4)==QString("停止报警"))
    {
        QJsonObject json;
        if(alerm_flag == 1)
        {
